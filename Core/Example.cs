...
namespace Core
{
    class Example
    {
        static void Main()
        {
            ...
            foreach (Image img in dbt.FindByTags("")) // my cat, my dog, the biggest cat
            {
                Console.WriteLine($"{img.Id}. {img.Name}");
            }

            foreach (Image img in dbt.FindByTags("cats")) // my cat, the biggest cat
            {
                Console.WriteLine($"{img.Id}. {img.Name}");
            }

            foreach (Image img in dbt.FindByTags("dogs")) // my dog
            {
                Console.WriteLine($"{img.Id}. {img.Name}");
            }

            foreach (Image img in dbt.FindByTags("animals")) // my cat, my dog, the biggest cat
            {
                Console.WriteLine($"{img.Id}. {img.Name}");
            }
        }
